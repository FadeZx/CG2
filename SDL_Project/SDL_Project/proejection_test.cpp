...
Affine cube2world[9],
snub2world;
float snub_rot_rate = 2 * PI / 10.0f;
Vector snub_rot_axis(0, 1, 0);
Point snub_center(0, 0, -2);
bool use_cam1 = true,
draw_solid = false;
...
void Init(void) {
    ...
    cam2 = cam2_0;
    float snub_scale = max(snub.Dimensions().x,
        max(snub.Dimensions().y, snub.Dimensions().z));
    snub2world = Trans(snub_center - O)
        * Scale(2.0f / snub_scale)
        * Trans(O - snub.Center());
    for (int i = 0; i < 9; ++i)
...
void Draw(void) {
    ...
    snub2world = Trans(snub_center - O)
        * Rot(snub_rot_rate * dt, snub_rot_axis)
        * Trans(O - snub_center)
        * snub2world;
    if (draw_solid)
        DisplayFaces(snub, snub2world, cam, Vector(0, 1, 0));
    else
        DisplayEdges(snub, snub2world, cam, Vector(0, 1, 0));
